...
fn test_late_bound_generics_assertion() -> anyhow::Result<()> {
        ...
        .filter(|item| {
            ...
            item.item_meta()
                ...
                && (name_str.contains("borrow_field1")
                    || name_str.contains("transform")
                    || name_str.contains("apply_fn"))
        })
        .collect();

    assert!(
        !monomorphized_items.is_empty(),
        "Should have monomorphized items with late-bound regions"
    );
    println!(
        "Found {} monomorphized items with potential late-bound regions",
        monomorphized_items.len()
    );

    // Test patterns that should work with late-bound regions
    let test_patterns = vec![
        (
            "test_crate::_::borrow_field1",
            "Should match monomorphized borrow_field1",
        ),
        (
            "test_crate::_::transform",
            "Should match monomorphized transform",
        ),
        (
            "test_crate::_::apply_fn",
            "Should match monomorphized apply_fn",
        ),
        (
            "_::_::borrow_field1",
            "Should match any borrow_field1 with glob",
        ),
    ];
    ...
    println!(
        "\n✅ Found {} items with late-bound regions",
        late_bound_examples_found
    );
