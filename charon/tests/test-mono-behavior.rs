...
fn test_monomorphized_pattern_matching() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Should have monomorphized 'new' methods"
    );
    ...
    for mono_item in &monomorphized_items {
        ...
        for (pattern_str, expected_regular, expected_mono) in &test_patterns {
            ...
            println!(
                "  Pattern '{}': regular={}, mono={} (expected: {}, {})",
                pattern_str, matches_regular, matches_mono, expected_regular, expected_mono
            );

            assert_eq!(
                matches_regular, *expected_regular,
                "Pattern '{}' regular matching failed for '{}'",
                pattern_str, name_str
            );
            assert_eq!(
                matches_mono, *expected_mono,
                "Pattern '{}' mono matching failed for '{}'",
                pattern_str, name_str
            );
        }
        ...
        assert!(
            name.name
                .iter()
                .any(|elem| matches!(elem, PathElem::Monomorphized(_))),
            "Item should contain a Monomorphized element: {}",
            name_str
        );
    }
    ...
    for regular_item in &regular_items {
        ...
        assert_eq!(
            ...
            "Mono matching should not affect regular items: {}",
            name_str
        );
    }
