...
pub enum Rvalue {
    ...
    /// The `Operand` refers to the init value of the metadata, it is `None` iff no metadata
    #[charon::rename("RvRef")]
    Ref {
        place: Place,
        kind: BorrowKind,
        ptr_metadata: Option<Operand>,
    },
    ...
    /// Like `Ref`, the `Operand` refers to the init value of the metadata, it is `None` iff no metadata.
    RawPtr {
        place: Place,
        kind: RefKind,
        ptr_metadata: Option<Operand>,
