    ...
    fn fmt_with_ctx_and_kind<C: AstFormatter>(
        ...
        for (local_id, rvalue) in self.captures.iter_enumerated() {
            writeln!(
                f,
                "{tab1}{}: {} = {},",
                local_id.with_ctx(ctx1),
                locals[local_id].ty.with_ctx(ctx1),
        ...
        write!(
            f,
            "{tab}]({})",
            self.non_captured_argument_ids()
                .map(|local_id| format!(
                    "{}: {}",
                    local_id.with_ctx(ctx),
                    locals[local_id].ty.with_ctx(ctx),
                ))
                .format(", "),
        )?;
        write!(f, "{}", self.body.with_ctx(ctx))
