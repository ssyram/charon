...
impl<'a> StorageVisitor<'a> {
    fn new(locals: &Locals, spec_closures: &'a IndexMap<SpecClosureId, SpecClosure>) -> Self {
...
impl<'a> VisitBody for StorageVisitor<'a> {
...
impl Transform {
    fn transform_ullbc_body(
        ...
        spec_closures: &IndexMap<SpecClosureId, SpecClosure>,
        insert_missing_storage_deads: bool,
    ) {
        let mut storage_visitor = StorageVisitor::new(&body.locals, spec_closures);
    ...
    fn transform_function(&self, ctx: &mut TransformCtx, fun: &mut FunDecl) {
        ...
        fun.for_each_body(
            ...
            |body, spec_closures| match body {
                Body::Unstructured(body) => {
                    self.transform_ullbc_body(body, spec_closures, insert_missing_storage_deads)
                }
                Body::Structured(body) => {
                    self.transform_llbc_body(body, spec_closures, insert_missing_storage_deads)
                }
                _ => {}
            },
        );
...
impl UllbcPass for Transform {
    fn transform_function(&self, ctx: &mut TransformCtx, fun: &mut FunDecl) {
        self.transform_function(ctx, fun)
    }
}
